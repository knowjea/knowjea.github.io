...
다른 사용방법으로는 네이티브 피어(native peer) 객체를 제거할 때 사용한다. 네이티브란 자바 외의 C나 C++로 작성된 프로그램을 나타낸다.
네이티브 피어란 C나 C++로 작성된 프로그램을 다루는 메소드를 가진 객체이다. 하지만 이런 객체는 실제로는 C나 C++로 작성된 프로그램을 다루기 때문에
...
하위 클래스에서 super.finalize를 잊으면 문제가 생긴다. 이러한 문제를 막기 위해 자동으로 finalize가 호출되도록 할 수 있다.

Finalizer Guardian 방식으로 상위 클래스에 private 필드로 익명클래스를 구현한다.
상위 클래스 객체가 종료되어야 할 때, 상위 클래스 객체가 참조하는 객체도 종료되어야 한다. 따라서 익명클래스가 종료되어야 하며
이때 익명클래스에 정의한 finalize가 호출된다. 따라서 익명클래스의 finalize 메소드 안에 마치 상위 클래스가 필요한 종료 작업을 작성하면 된다.
